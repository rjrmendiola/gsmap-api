...
const { WeatherSnapshot } = require('../models');

router.get('/', async (req, res) => {
  try {
    const WeatherSnapshots = await WeatherSnapshot.findAll();
...
router.post('/', async (req, res) => {
  try {
    ...
    const WeatherSnapshot = await WeatherSnapshot.create({ refresh_interval_minutes });
    res.status(201).json(WeatherSnapshot);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
